...
All options here are provided through the `morsels.initMorsels` function exposed by the search bundle.
...
```ts
const { showFullscreen, hideFullscreen } = morsels.initMorsels({ ... });
```

You may call the `showFullscreen()` function returned by the `morsels.initMorsels` call to programatically show the fullscreen search UI.
