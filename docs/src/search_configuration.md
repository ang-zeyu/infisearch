...
| `"dropdown"`    | This wraps the specified `input` element with a root container. Search results are displayed using an additional container attached to this root container.<br><br>**Styling Note:**<br>When using this option, it is important to note that the dropdown may overflow your screen depending on your dropdown alignment and screen size (more likely for mobile devices). You will need to adjust the maximum width accordingly (see [css styling](https://github.com/ang-zeyu/morsels/blob/main/packages/search-ui/src/styles/search.css)). This may likely change in the short future using [popper.js](https://popper.js.org/). As such, intrusive modifications (mainly renderers, covered in the next page) relying on current dropdown behaviours are discouraged.    |
...
| ----------- | -----------           | -----------      | ----------- |
| `dropdown`  | `dropdownAlignment`   | `'right'`        | `'left'` or `'right'`. Which side of the input element to align the dropdown results container against. 
