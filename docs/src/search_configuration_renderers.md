...
| `loadingIndicatorRender(h, opts, isInitialising: boolean, initialisePromise: Promise<any>)` | `HTMLElement`  | This API renders the loading indicator attached under the `listContainer`. The loading indicator is shown when making the initial search (the first search from an empty search box).<br><br>While the search library is initialising, the `isInitialising` parameter will be `true`. You may use the `initialisePromise` to make modifications to the rendered content once the library is initialised (and the query has begun).   |
