...
The settings here follow from the section directly above, disregarding the assumptions. That is,
...
There are 2 other options especially worth highlighting that can help reduce the index size, both of which are not active by default (this may change in the future).
...
> ⚠️ Most static file servers only gzip specific mime types. As the index files generated by morsels are of type `application/octet-stream`, you will need to configure the static file server as appropriate!
